...
function wallet_address(string $address): string {
    $address = strtolower(trim($address));
    if (!preg_match('/^0x[a-f0-9]{40}$/', $address)) {
        wallet_json(['status' => 'error', 'message' => 'Valid EVM wallet address is required'], 400);
    }
    return $address;
}
...
try {
    ...
    $address = wallet_address($data['wallet_address'] ?? '');
    $walletType = preg_replace('/[^a-z0-9_-]/i', '', $data['wallet_type'] ?? 'evm') ?: 'evm';
    ...
    $userId = (int) $user['id'];
    $stmt = $conn->prepare(
        'INSERT INTO user_wallets (user_id, wallet_address, wallet_type, created_at, updated_at)
         VALUES (?, ?, ?, NOW(), NOW())
         ON DUPLICATE KEY UPDATE wallet_address = VALUES(wallet_address), wallet_type = VALUES(wallet_type), updated_at = NOW()'
    );
    $stmt->bind_param('iss', $userId, $address, $walletType);
    $stmt->execute();

    wallet_json(['status' => 'success', 'wallet_address' => $address, 'wallet_type' => $walletType]);
} catch (Throwable $e) {
    error_log('save_wallet.php: ' . $e->getMessage());
    wallet_json(['status' => 'error', 'message' => 'Wallet save failed'], 500);
}
?>
